...
//                        $mail->addAddress($email->toEmail, $email->toName);
                        ...
                        if ($email->lang == 'hu')
                        {
                            $mail->setFrom('mikpartners@mik.pte.hu', 'MIK Nyíltnap Regisztráció');
                            $mail->Subject = 'Sikeres regisztráció';
                            $mail->AltBody = 'MIK Nyíltnap Regisztráció';
                        }else if($email->lang='en')
                        {
                            $mail->setFrom('mikpartners@mik.pte.hu', 'UP FEIT Opening of the academic year registration');
                            $mail->Subject = 'Successful registration!';
                            $mail->AltBody = 'UP FEIT Opening of the academic year registration';
                        }
